...
class TestMarginCascade:
    ...
    def test_cascade_recovers_when_margin_improves(self):
        ...
        # used_margin = 200000 * 0.02 = 4000
        # level = (2000 / 4000) * 100 = 50% (at the boundary)
        status = margin_engine.check(account, broker_profile, 200000.0)
        ...
        status = margin_engine.check(account, broker_profile, reduced_market_value)
    ...
    def test_margin_status_transitions(self):
        ...
        # used_margin = 20000 * 0.05 = 1000
        # level = (10000 / 1000) * 100 = 1000%
        status = margin_engine.check(account, broker_profile, 20000.0)
        assert status.status == "ok"
        assert status.level == 1000.0

        # Test 2: Margin call with 200000 market value
        # used_margin = 200000 * 0.05 = 10000
        ...
        # used_margin = 400000 * 0.05 = 20000
        # level = (10000 / 20000) * 100 = 50%
        status = margin_engine.check(account, broker_profile, 400000.0)
        assert status.status == "stop_out"
        assert status.level == 50.0

        # Test 4: Below stop-out with 500000 market value
        # used_margin = 500000 * 0.05 = 25000
        # level = (10000 / 25000) * 100 = 40%
        status = margin_engine.check(account, broker_profile, 500000.0)
        assert status.status == "stop_out"
